...
4. **🟡 Dynamic/Yield Pricing Engine v1 — BUILT 2026-09-20 (migration 061), DEPLOYED to production 2026-09-21 (module `yield_management` is OFF by default, so no production rate has changed). Verified against the live dev DB + real HTTP round-trips; client verified by build only (not clicked through in a browser).** Design decisions locked with the owner:
   ...
   - **api.co.id holiday sync — implemented 2026-09-21 (no new migration; reuses migration 061's `holidays.source`/`is_joint_leave` columns).** `server/services/holidayApi.js` (base URL `https://use.api.co.id`, confirmed live via the owner's dashboard — the static docs page renders its base URL client-side and doesn't expose it any other way; header `x-api-co-id: HOLIDAY_API_KEY`) + `jobs/marketInsights.js`'s `refreshHolidays()`, cron daily 01:30 WITA (before Yield's 02:00 run so that night's rates see fresh data), manual trigger `POST /api/insights/holidays/refresh` (owner-only). Fetches current year + 2 years ahead, keeps only `is_holiday`/`is_joint_holiday` rows (the API's "Observance"/awareness-day rows — Hari Kartini etc. — are dropped, they don't move guest demand), replaces by **exact date**, `source='api_co_id'`. **Caught and fixed a real bug while verifying against the live dev DB**: an early version deleted by whole-year range, which wiped the entire migration-061 `skb` 2027 fallback seed the moment the sync ran, even though api.co.id has **no 2027 data published yet** (confirmed live: `year=2027` returns `is_success:true, data:[]`, and `check/date?date=2027-08-17` — a real national holiday every year — returns `is_holiday:false`) — leaving 2027 yield pricing with zero holiday uplift instead of the old hardcoded fallback. Fixed to delete/replace only the exact dates a fetch actually returns, so a year the API hasn't caught up on yet is left alone; the `skb` seed keeps serving as a per-date fallback until the live API actually covers that date, at which point the same nightly sync supersedes it automatically, no code change needed. Balinese Hindu holidays (pawukon calendar, not in any generic holiday API) stay manually seeded — still only 2027 (Galungan/Kuningan/Nyepi). Verified against the live dev DB: idempotent re-run produces no duplicate dates, 2026 fully live-synced, 2027 SKB fallback intact. Allotments (agent room blocks) are not counted in occupancy. Room-type out-of-order is current-status only, not date-based.
...
are the new paid add-on module `smart_reports` (default off). A property's own recipient list
(`notification_recipients` — any email or Telegram chat, with or without a PMS login, with a
role: owner / manager / front desk / kitchen / other) and a tick list of reports per
recipient. Telegram recipients connect themselves via a Connect link (QR / WhatsApp share, press
Start) — no Chat ID needed. The night-audit owner email is unchanged.

Agreed plan (English only; times WITA):
