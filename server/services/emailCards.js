...
const CARD_HEIGHT = { none: 62, one: 80, two: 98 };
...
function card(label, value, sub = '', { height = CARD_HEIGHT.one, valueColor = '#111827' } = {}) {
  ...
  return `
    <td width="33%" height="${height}" valign="top" style="width:33.33%;height:${height}px;vertical-align:top;background:#f9fafb;border:1px solid #e5e7eb;border-radius:10px;padding:14px 16px;">
      ...
      <div style="font-size:20px;line-height:26px;font-weight:700;color:${valueColor};white-space:nowrap;">${value}</div>
      ${lines}
    </td>`;
}

module.exports = { CARD_TABLE_OPEN, CARD_TABLE_CLOSE, CARD_HEIGHT, card, subLine };
