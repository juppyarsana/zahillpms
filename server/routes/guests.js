...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    const staysQ = db.query(`
      ...
      WHERE b.guest_id = $1
      ORDER BY b.check_in_date DESC`, [req.params.id]);
...
router.post('/', auth, async (req, res) => {
  ...
  try {
    const { rows } = await db.query(
      `INSERT INTO guests (name, nationality, whatsapp, email, birthday, anniversary, notes)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [name, nationality, whatsapp, email, birthday || null, anniversary || null, notes]
    );
...
router.put('/:id', auth, async (req, res) => {
  ...
  try {
    ...
    const { rows } = await client.query(
      ...
      [name, nationality, whatsapp, email, birthday || null, anniversary || null, notes, req.params.id]
    );
