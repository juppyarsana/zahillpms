...
router.post('/', auth, requireRole('owner'), async (req, res) => {
  ...
  try {
    const { rows } = await db.query(
      'INSERT INTO products (name, category, price, description, property_id) VALUES ($1,$2,$3,$4,$5) RETURNING *',
      [name, category || 'other', price, description, req.propertyId]
    );
...
router.put('/:id', auth, requireRole('owner'), async (req, res) => {
  ...
  try {
    const { rows } = await db.query(
      ...
      [name, category, price, description, is_available, req.params.id, req.propertyId]
    );
