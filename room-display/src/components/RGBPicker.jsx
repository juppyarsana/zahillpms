    ...
    <div className="glass-card rounded-3xl p-6 flex flex-col gap-4">
      <div className="flex items-center gap-3 mb-1">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center accent-tint">
          <span className="material-symbols-outlined text-xl text-accent">palette</span>
        </div>
        <div>
          <h3 className="text-sm font-semibold text-ink">Ambient Light</h3>
          <p className="text-[10px] text-dim">RGB LED strip color</p>
        </div>
      </div>
      <div className="grid grid-cols-4 gap-2">
        {PRESETS.map(preset => (
          ...
          >
            ...
            />
            <span className="text-[9px] text-muted font-medium leading-tight text-center">{preset.name}</span>
          </button>
        ))}
      </div>
    </div>
