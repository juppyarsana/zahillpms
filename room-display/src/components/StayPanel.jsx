...
export default function StayPanel({
  ...
  return (
    <section
      className="w-[420px] shrink-0 flex flex-col border-r border-app-soft p-10 bg-pane overflow-y-auto"
    >
      <header className="mb-7">
        <p className="text-xs font-bold uppercase tracking-[0.3em] mb-2 text-accent">Reservation</p>
        <h2 className="text-3xl font-extralight text-ink leading-tight">Your Stay</h2>
        <p className="text-muted mt-1.5 text-sm">Welcome, {booking.guest_name}</p>
      </header>

      <div className="flex-1 flex flex-col gap-5">
        {/* Unit card */}
        <div
          className="relative h-44 rounded-3xl overflow-hidden flex items-end"
          style={{
        ...
        >
          ...
          <span
            ...
            style={{ top: 10, right: 14, fontSize: '6.5rem', opacity: 0.16, transform: 'rotate(-8deg)' }}
          >
            villa
          </span>
          <div
          ...
          >
            <span
              className="inline-flex items-center px-5 py-2.5 rounded-full text-lg font-extrabold uppercase tracking-widest mb-3 w-fit bg-accent"
              style={{ color: 'var(--accent-contrast)', boxShadow: '0 4px 14px rgb(var(--accent-rgb) / 0.4)' }}
            >
              {unit.name}
            </span>
            <h3 className="text-2xl text-ink" style={{ fontFamily: 'var(--font-brand)', fontWeight: 700 }}>{property?.name || ''}</h3>
            {property?.location && (
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="glass-card p-6 rounded-2xl">
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-dim mb-5">Check-in</p>
            <p className="text-3xl font-light text-ink">{fmt(booking.check_in_date)}</p>
            <p className="text-[11px] mt-2.5 text-accent">After 14:00</p>
          </div>
          <div className="glass-card p-6 rounded-2xl">
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-dim mb-5">Check-out</p>
            <p className="text-3xl font-light text-ink">{fmt(booking.check_out_date)}</p>
            <p className="text-[11px] mt-2.5 text-accent">Before 12:00</p>
          </div>
        </div>
