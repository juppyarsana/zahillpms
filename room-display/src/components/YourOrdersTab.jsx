  ...
  return (
    <section className="flex-1 p-10 bg-app overflow-y-auto">
      <div className="mb-8 flex items-start justify-between">
        <div>
          <h2 className="text-3xl font-extralight text-ink mb-1">Your Orders</h2>
          <p className="text-dim text-sm">Food orders and activity bookings for this stay.</p>
        </div>
        <button
          onClick={onRefresh}
          className="w-9 h-9 rounded-full flex items-center justify-center text-muted bg-surface-2"
      ...
      {empty && <p className="text-dim text-sm">Nothing ordered or booked yet.</p>}
      ...
      {activityBookings.length > 0 && (
        <div>
          <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-muted mb-4">Activity Bookings</h3>
          <div className="flex flex-col gap-3">
            {activityBookings.map(b => {
              ...
              return (
                <div key={b.id} className="glass-card rounded-2xl p-5">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <span className="text-ink text-sm font-medium">{b.activity_name}</span>
                    <StatusBadge {...status} />
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-dim text-xs">
                    ...
                    <span className="text-muted text-sm">{fmtIDR(b.total_amount)}</span>
                  </div>
                </div>
              );
            })}
