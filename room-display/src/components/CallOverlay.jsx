  ...
  if (status === 'missed' || status === 'ended' || status === 'failed') {
    return (
      <div style={{
        ...
        background: 'var(--pane)', border: '1px solid rgb(var(--accent-rgb) / 0.35)',
        borderRadius: 16, padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 12,
        boxShadow: '0 8px 32px rgba(0,0,0,0.4)',
      }}>
        <span className="material-symbols-outlined text-accent" style={{ fontSize: 22 }}>call_end</span>
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center z-50" style={{ background: 'var(--overlay)', backdropFilter: 'blur(6px)' }}>
        <div className="flex flex-col items-center gap-6 text-center">
          <div style={{
            width: 96, height: 96, borderRadius: '50%', background: 'var(--danger-soft)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>
            <span className="material-symbols-outlined" style={{ fontSize: 48, color: 'var(--danger)' }}>call</span>
          </div>
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.3em] mb-2" style={{ color: 'var(--danger)' }}>Incoming Call</p>
            <h1 className="text-2xl font-extralight text-ink">Front Desk</h1>
          </div>
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50" style={{ background: 'var(--overlay)', backdropFilter: 'blur(6px)' }}>
      <div className="flex flex-col items-center gap-6 text-center">
        <div style={{
          ...
          <span className="material-symbols-outlined text-accent" style={{ fontSize: 48 }}>call</span>
        </div>
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.3em] mb-2 text-accent">Front Desk</p>
          <h1 className="text-2xl font-extralight text-ink">
...
function btnStyle(danger) {
  return {
    ...
    color: danger ? '#fff' : 'var(--text)', display: 'flex', alignItems: 'center', justifyContent: 'center',
  };
}
...
function CallTimer() {
  ...
  return <p className="text-dim text-sm mt-1">{m}:{s}</p>;
}
