...
| `calling` | calls | ✅ on |
| `resto_ordering` | resto, restoGuest | ❌ off by default (paid add-on tier, same philosophy as the planned `back_office` module) |
...
**Next migration number: 052** (keep `ROADMAP.md` in sync when you add one — this line was found stale at 047 when 047 already existed on disk; double-check against the actual highest-numbered file in `server/db/migrations/` if in doubt, don't trust this line blindly).
...
- **Housekeeping room status — checkout-cleaning flow (2026-09-09, migration 051).** Front desk previously could not tell a just-vacated uncleaned room from a ready one — checkout set `units.status='available'` immediately, and housekeeping lived only as a `tasks` kanban card with no effect on room state. Added `units.housekeeping_status` (`clean`/`dirty`; `inspected` reserved in the CHECK, unused) as a **second dimension kept fully separate from `units.status`** — a room can be `available` AND `dirty`. Folding it into `units.status` would have broken every `WHERE status='occupied'`/`'available'` query. Scope (per owner): 2 states only, no attribution/login, checkout-cleaning only (no stayover/daily service), dirty check-in warns not blocks.
  ...
  - **PMS client:** Dashboard Live Unit Status — new `'dirty'` tile state (amber `#D97706` + 🧹, wins over `available`/`arriving` when vacant), split "Available" strip count into clean-only + a new "To clean" count. `UnitCard` shows "🧹 Vacant — needs cleaning" + an inline **Mark clean** (→ confirm → `PATCH /api/units/:id/housekeeping`), a small "Flag for cleaning" link on ready rooms for the reverse, and calls `onChanged` (threaded `UnitStatusBoard`→`UnitPopover`→`UnitCard`, wired to the Dashboard's `load()`). `CheckIn.jsx` arrivals list shows an amber "🧹 Room not yet cleaned" line (non-blocking).
