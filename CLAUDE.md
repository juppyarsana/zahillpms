...
- `iot`, `calls` — Room Controller (MQTT-backed device state, two-way room↔desk voice calls — see the Room Display section below for the full write-up)
...
**Next migration number: 040** (keep `ROADMAP.md` in sync when you add one).
...
- ✅ Two-way room-to-desk calling — implemented, migration 039. Staff can now call a room, not just answer one. Not yet manually verified in a browser.
- 🔵 Phase D item 13 (Agent Accounts / Direct Billing — company, travel agent, wholesaler) — scoped 2026-08-17, revised same day after reviewing guestpro.id as a reference PMS. Not started. Generalizes the existing `booking_sources` table (adds `source_type` + `payment_status` city-ledger/commission flow) instead of a standalone companies table — full spec in `ROADMAP.md`. Open questions for the client: credit terms, credit-limit enforcement, Faktur Pajak invoice format.
