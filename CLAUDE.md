...
**Next migration number: 050** (keep `ROADMAP.md` in sync when you add one — this line was found stale at 047 when 047 already existed on disk; double-check against the actual highest-numbered file in `server/db/migrations/` if in doubt, don't trust this line blindly).
...
- **Table sessions** (`table_sessions`, migration 048) group every order from one dining visit so a table's QR always resolves to whatever session is currently active — an old scan can't leak into the next party's visit. A session opens implicitly on a table's first order (guest QR or staff) or explicitly via staff "Seat Party," and closes only via staff "Close Table" (`server/services/tableSessionService.js`) — **manual only, no auto-close job**, a deliberate trade-off accepted knowingly: a forgotten table stays "occupied" until a staff member clears it. `restaurant_tables.status` (available/occupied) is kept as a derived flag the session service writes, not a second source of truth.
- **Payment methods:** this pass also dropped `sales.payment_method`'s old hardcoded `cash`/`qris`/`room_charge` CHECK constraint (from migration 001) and widened the column to `VARCHAR(50)` — the resto app can now accept any of a property's configured `payment_methods`. Validation moved into `salesService.createSale` (the one choke point every caller — staff POS, Room Display, resto app — goes through), which special-cases `room_charge` (not a real `payment_methods` row; means "post to the guest's folio," requires a `booking_id`) and otherwise checks the property's real `payment_methods` table.
