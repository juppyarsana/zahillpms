...
**Next migration number: 043** (keep `ROADMAP.md` in sync when you add one).
...
- ⏳ Phase D item 13 (Agent Accounts / Direct Billing — company, travel agent, wholesaler) — **Slices A + B shipped.** Slice A (migration 041): `booking_sources` doubles as the per-property agent registry (`source_type`/`payment_status`/billing fields). Slice B (migration 042): `bookings.folio_status` + `agent_commissions` ledger; `checkin.js`'s checkout endpoint takes an optional `bill_to_agent` flag and calls `server/services/agentBillingService.js` — `settleCheckout()` marks the folio `pending_agent_invoice` for city-ledger sources and posts a one-per-booking commission row for `commission`/`commission_and_city_ledger` sources (computed at checkout only). `loadFolio` moved to `server/services/folioService.js`. Non-blocking credit-limit warning (`GET /api/settings/booking-sources/:id/credit-check?amount=`) in `NewBooking.jsx` + the checkout modal. Client decisions locked: credit limit = warn only, Faktur Pajak = deferred, agent payment allocation = auto oldest-first with manual adjust (Slice C). **Slice C** (per-agent statement/aging, consolidated invoice PDF, `POST .../payments`, AR Aging report) not started. Full spec in `ROADMAP.md`.
